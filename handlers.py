...
async def chat(message: Message):
    ...
    try:
        ...
        prompt = f"""
          ...
          Дай краткий анализ (до 400 слов):
            1. Топ-3 категории трат
            2. 2-3 практичных совета по экономии
            3. Общая оценка финансового поведения
            
            КРИТИЧЕСКИ ВАЖНЫЕ ПРАВИЛА ФОРМАТИРОВАНИЯ:
            - Используй ТОЛЬКО эти HTML теги: <b>текст</b> для выделения
            - НЕ используй звездочки (*), подчеркивания (_), или другой Markdown
            - НЕ используй символы < и > нигде кроме HTML тегов
            - Каждый открывающий тег <b> должен иметь закрывающий </b>
            - Проценты пиши так: "50%" (без дополнительных символов)
            - Используй эмодзи для визуального разделения
            - Пиши дружелюбно и честно, но кратко
            ...
            НЕ делай так:
            - **текст** или *текст* (это Markdown)
            - текст < 100 или текст > 50 (используй "меньше" и "больше" словами)
            - незакрытые теги <b>текст
          """
